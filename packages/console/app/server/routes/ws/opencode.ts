  ...
  async open(peer: any) {
    ...
    const pty = await sandbox.process
      ...
      .catch((error) => {
        ...
        safeSend(peer, {
          type: "error",
          data: message.includes("/usr/bin/zsh")
            ? "Demo sandbox is missing zsh; rebuild the Daytona snapshot image."
            : "Failed to start demo terminal.",
        })
    ...
    await pty.sendInput("cd /home/daytona/opencode && exec opencode; exit\n").catch(() => {})
